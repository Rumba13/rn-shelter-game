            ...
            <ScrollView>
              <ImageBackground
                ...
                <GameOptionList<ShelterCategoryList>
                  ...
                  onValueChange={shelterCategories =>
                    gameCreationOptionsModel.setOptions(options => {
                      const sheltersNames: string[] = shelterCategoriesToShelterNames(shelterCategories)
                      options.shelters = sheltersNames.map(shelterName => shelterNameToShelter(shelterName))
                    })
                  }
                />
                <GameOptionList<ApocalypseCategories> title={'Список апокалипсисов'} descriptionHeight={100}
                                                      ...
                                                      onValueChange={(apocalypsesNames) => gameCreationOptionsModel.setOptions(options => {
                                                        const apocalypseNames: string[] = apocalypseCategoriesToApocalypseNames(apocalypsesNames)
                                                        options.apocalypses = apocalypseNames.map(apocalypseName => apocalypseNameToApocalypse(apocalypseName))
                                                      })}
                                                      subKey={'children'} />
              </ImageBackground>
            </ScrollView>
